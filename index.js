...
const ScrollableTabView = createReactClass({
  ...
  componentWillReceiveProps(props) {
    if (props.children !== this.props.children) {
      this.updateSceneKeys(
        { page: this.state.currentPage, children: props.children, });
    }
    ...
    if (Platform.OS === 'ios') {
      ...
      if (this.scrollView) {
        this.scrollView.getNode().scrollTo(
          { x: offset, y: 0, animated: !this.props.scrollWithoutAnimation, });
      }
    } else {
      if (this.scrollView) {
  ...
  updateSceneKeys({ page, children = this.props.children, callback = () => {}, }) {
    let newKeys = this.newSceneKeys(
      { previousKeys: this.state.sceneKeys, currentPage: page, children, });
  ...
  _handleLayout(e) {
    ...
    if (!width || width <= 0 || Math.round(width) === Math.round(
    ...
    if (Platform.OS === 'ios') {
      ...
      // https://github.com/facebook/react-native/pull/14435
      containerWidthAnimatedValue.__makeNative();
      scrollValue = Animated.divide(this.state.scrollXIOS,
  ...
  render() {
    let overlayTabs = (this.props.tabBarPosition === 'overlayTop'
